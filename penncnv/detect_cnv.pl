...
our ($train, $ctrain, $test, $hmmfile, $region, $loh, $minsnp, $minlength, $minconf,
	...
	$startsnp, $endsnp, $candlist, $bafxhet_threshold, $valilog, $cnprior, $tseq, $gamma, $paramfile, $lastchr, $cache);
...
sub processArgument {
	GetOptions ('verbose|v'=>\$verbose, 'help|h'=>\$help, 'man|m'=>\$man, 
		...
		'tseq'=>\$tseq, 'gamma=s'=>\$gamma, 'paramfile=s'=>\$paramfile, 'lastchr=i'=>\$lastchr, 'cache'=>\$cache,
	) or pod2usage ();
	...
	if ($logfile) {
		...
		if ($msg =~ m/tee/) {								#system error message should contain "tee" word
			$notee++;
		} else {
			eval {
